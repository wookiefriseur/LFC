...
GHLIST_STATUS_PRERELEASE = 'Pre-release'
GHLIST_STATUS_DRAFT = 'Draft'
GHLIST_STATUSES_EXCLUDED = (GHLIST_STATUS_PRERELEASE, GHLIST_STATUS_DRAFT)
"""Don't accidentally use a draft or pre-release for ESOUI"""

def get_highest_version_from_gh_list(gh_list :str) -> str:
  """Get the highest release version from a list generated by gh cli

  ```
  Input example (tabs shown as spaces):
  ...
  for entry in gh_list.split('\n'):
    fields = entry.split('\t')
    if len(fields) < 3: continue # not a release row (e.g. blank line)
    status = fields[1].strip()
    if status in GHLIST_STATUSES_EXCLUDED: continue
